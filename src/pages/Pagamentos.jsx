  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-[1600px] mx-auto px-4 py-6">
        {/* Header */}
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-brand-800">Gestão de Pagamentos</h1>
          <p className="text-sm text-gray-500 mt-1">Folhas de ponto, recibos e histórico de pagamentos</p>
        </div>

        {/* Tabs */}
        <div className="flex gap-1 bg-white border rounded-xl p-1 shadow-sm mb-6 overflow-x-auto">
          {tabs.map(t=>(
            <button key={t.id} onClick={()=>setTab(t.id)} className={`flex-1 min-w-max px-4 py-2 rounded-lg text-sm font-medium transition-colors ${tab===t.id?'bg-brand-600 text-white shadow-sm':'text-gray-600 hover:bg-gray-100'}`}>
              {t.label}
            </button>
          ))}
        </div>
